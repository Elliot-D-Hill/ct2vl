...
class CT2VL:
    ...
    def __post_init__(self, traces):
        if isinstance(traces, ndarray):
            traces = DataFrame(traces)
        if not isinstance(traces, DataFrame):
            ValueError('traces must be a pandas dataframe or numpy ndarray')
        self.calibrate(traces)

    def fit_model(self, ct_values, max_efficiency):
        ...
        self.slopes = array([slope, low_95ci_slope, high_95ci_slope])
        self.intercepts = array([intercept, low_95ci_intercept, high_95ci_intercept])

    def calibrate(self, traces):
    ...
    def convert(self, Ct):
        ...
        viral_load = self.ct_value_to_viral_load(Ct)
        return format_results(Ct, viral_load)
