...
preprocess_traces_output = DataFrame(
    {0: [1, 3, 3, 4], 1: [2, 2, 2, 3], 2: [1, 1, 2, 2]}
)

get_max_replication_rate_input = DataFrame({0: [1, 2, 8, 10], 1: [1, 5, 6, 10]})
...
configure_arguments_calibrate_input = ["calibrate", "test.tsv", "100.0", "37.83"]
configure_arguments_calibrate_output = Namespace(
    mode="calibrate", infile="test.tsv", LoD=100.0, Ct_at_LoD=37.83
)

configure_arguments_convert_input = [
    "convert",
    "37.83",
    "--outfile",
    "test.tsv",
...
configure_arguments_convert_output = Namespace(
    mode="convert", Ct=[37.83], outfile="test.tsv"
)
